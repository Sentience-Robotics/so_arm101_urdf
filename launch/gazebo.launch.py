...
_PKG_NAME = "so_arm101_urdf"


def _gz_ros2_control_plugin_path():
    """Lib dir for gz_ros2_control — prefer ament prefix over distro hardcoding."""
    try:
        return os.path.join(get_package_prefix("gz_ros2_control"), "lib")
    except Exception:
        try:
...
def _infer_robot_source_root(pkg_share: str) -> Path | None:
    """Prefer workspace ``src/<pkg>`` (writable) over install share when present."""
    cwd_candidate = Path.cwd() / "src" / _PKG_NAME
    if cwd_candidate.is_dir():
        return cwd_candidate
    share = Path(pkg_share)
    for p in share.parents:
        if p.name == "install":
            src_candidate = p.parent / "src" / _PKG_NAME
...
def _active_generated_files(pkg_share: str) -> dict[str, str]:
    ...
    src_root = _infer_robot_source_root(pkg_share)
    candidates = []
    if src_root is not None:
        candidates.append(src_root / "config" / "hardware" / "active.yaml")
    candidates.append(Path(pkg_share) / "config" / "hardware" / "active.yaml")
...
def _default_controllers_yaml(pkg_share: str, controllers_basename: str) -> str:
    src_root = _infer_robot_source_root(pkg_share)
    if src_root is not None:
        cwd_candidate = src_root / "config" / controllers_basename
...
def generate_launch_description():
    ...
    pkg_share = get_package_share_directory("so_arm101_urdf")
    ...
    bridge_config_path = os.path.join(
        get_package_share_directory("so_arm101_urdf"),
        "description",
        "gazebo",
        "gazebo_bridge.yaml",
    )
    ...
    try:
        ...
        raise RuntimeError(
            "ros_gz_sim package is required for gazebo.launch.py "
            "(install ros-jazzy-ros-gz-sim or source the workspace)"
        ) from e
    ...
    spawn_robot = TimerAction(
        ...
        actions=[
            Node(
                package="ros_gz_sim",
                executable="create",
                arguments=["-name", "so_arm101", "-topic", "robot_description", "-z", "0.0"],
                output="screen",
                parameters=[{"use_sim_time": True}],
            )
        ],
    )
    mesh_dae = get_package_share_directory("so_arm101_urdf")
